...
// Удалено: константа PERSISTENCE_FILE теперь динамическая в классе TorrentManager
...
export class TorrentManager extends EventEmitter {
    constructor(storagePath = './nectar-data') {
        ...
        this.savedTorrents = new Map() // key -> saved torrent metadata for persistence

        // Файл состояния теперь находится в директории данных
        this.persistenceFile = path.join(this.storagePath, 'torrents-state.json')

        // Инициализируем менеджер рейтингов
        this.ratingManager = new RatingManager(this.rootStore, this.swarm, this.storagePath)

        // Инициализируем менеджер поиска
        this.searchManager = new SearchManager(this.rootStore, this.swarm)
        ...
        // Загружаем сохранённые торренты при инициализации
        this._loadState()
    }
    ...
    async seed(sourcePath, customName = null) {
        ...
        try {
            // Сначала записываем манифест (метаданные)
            ...
            await new Promise((resolve, reject) => {
                ...
                manifestStream.on('finish', () => {
                    console.log(`[Seed] Manifest written successfully`)
                    resolve()
                })
                manifestStream.on('error', (err) => {
                    console.error(`[Seed] Manifest write error:`, err.message)
                    reject(err)
                })
            ...
            console.log(`[Seed] Manifest flushed to Hypercore, version: ${drive.version}`)
            ...
            // Добавляем в каталог поиска
            this.searchManager.addToCatalog(torrent)

            this.emit('update', this.getTorrents())
            this._saveState() // Сохраняем состояние после успешного создания сида
            return key
    ...
    async download(keyHex, saveDir = './downloads') {
        ...
        this._saveState() // Сохраняем состояние после начала загрузки
    ...
    async confirmDownload(keyHex, selectedFiles = null) {
        ...
        this.emit('update', this.getTorrents())
        this._saveState() // Сохраняем состояние после подтверждения загрузки
    ...
    async remove(keyHex) {
        ...
        // Удаляем из каталога поиска
        this.searchManager.removeFromCatalog(keyHex)
    ...
    async _fetchMetadata(keyHex, updateType = true) {
        ...
        try {
            ...
            // Сначала читаем манифест с retry-логикой
            let metadata = null
            console.log(`[Download] Fetching metadata...`)
            for (let i = 0; i < 20 && !metadata; i++) {
                await drive.core.update({ signal })
                try {
                    // Пробуем manifest.json (новый формат) или .metadata.json (старый)
                    ...
                    if (metadataEntry) {
                        ...
                        console.log(`[Download] Metadata loaded from ${manifestPath}: ${JSON.stringify(metadata)}`)
                    }
                } catch (err) {
            ...
            while (entries.length === 0 && this.torrents.has(keyHex)) {
                ...
                for await (const entry of drive.list()) {
                    if (entry.value.blob && entry.key !== '/.metadata.json' && entry.key !== '/manifest.json') {
                        entries.push({
                            path: entry.key,
            ...
            torrent.appliedVersion = drive.version // Set applied version after metadata fetch
    ...
    async _startDownload(keyHex, filesOverride = null) {
        ...
        try {
            ...
            for (const entry of filesToDownload) {
                ...
                if (fs.existsSync(fullPath)) {
                    ...
                    if (stats.size === entry.size) {
                        console.log(`[Download] Skipping existing file: ${filePath}`)
            ...
            torrent.progress = 100
            torrent.activeTransfer = false
            torrent.appliedVersion = drive.version // Fix applied version after download
            this.emit('update', this.getTorrents())
            this._saveState() // Сохраняем состояние после завершения загрузки

            // Автоматически оцениваем пира после успешной загрузки
            this._autoRateSeeder(keyHex)
    ...
    async _autoRateSeeder(keyHex) {
        const torrent = this.torrents.get(keyHex)
        if (!torrent) return

        // Получаем ключ сида из drive
        const seederKey = torrent.drive.key.toString('hex')

        // Обновляем статистику
        this.ratingManager.updatePeerStats?.(seederKey, { downloads: true })

        console.log(`[Rating] Seeder ${seederKey.slice(0, 8)}... downloaded from`)
    }
    ...
    _monitorRemoteUpdates(keyHex) {
        ...
        console.log(`[Monitor] Started monitoring for ${keyHex}`)
        ...
        torrent.updateInterval = setInterval(async () => {
            ...
            try {
                ...
                // If we see a version higher than what we last "applied"
    ...
    _saveState() {
        try {
            ...
            for (const [key, torrent] of this.torrents) {
                // Сохраняем только метаданные, не активные соединения
                state.torrents.push({
                    key: torrent.key,
                    name: torrent.name,
                    type: torrent.type,
                    progress: torrent.progress, // Сохраняем прогресс
                    sourcePath: torrent.sourcePath, // Для сидов
                    saveDir: torrent.saveDir, // Для загрузок
                    selectedFiles: torrent.selectedFiles, // Выбранные файлы
                    fileCount: torrent.fileCount,
                    metadata: torrent.metadata,
                    files: torrent.files,
                    size: torrent.size,
                    createdAt: torrent.createdAt || new Date().toISOString(),
                    // Не сохраняем: drive, store, discovery, controller, activeTransfer
                })
            }

            // Также сохраняем торренты которые были в metadata-ready состоянии
            for (const [key, saved] of this.savedTorrents) {
                if (!state.torrents.find(t => t.key === key)) {
                    state.torrents.push(saved)
                }
            }
            ...
            console.log(`[Persistence] Saved ${state.torrents.length} torrents to ${this.persistenceFile}`)
        } catch (err) {
    ...
    async _loadState() {
        try {
            ...
            console.log(`[Persistence] Loading ${state.torrents.length} torrents from ${this.persistenceFile}`)

            for (const torrentData of state.torrents) {
                this.savedTorrents.set(torrentData.key, torrentData)
            }
    ...
    async _restoreTorrents() {
        try {
            let restored = 0

            for (const [key, torrentData] of this.savedTorrents) {
                // Пропускаем уже активные торренты
                if (this.torrents.has(key)) continue

                try {
                    if (torrentData.sourcePath) {
                        // Это сид - восстанавливаем
                        console.log(`[Restore] Restoring seed: ${torrentData.name}`)
                        await this._restoreSeed(torrentData)
                        restored++
                    } else if (torrentData.saveDir) {
                        // Это загрузка - восстанавливаем
                        console.log(`[Restore] Restoring download: ${torrentData.name}`)
                        await this._restoreDownload(torrentData)
                        restored++
                    }
                } catch (err) {
                    console.error(`[Restore] Failed to restore ${key}:`, err.message)
                }
            }

            if (restored > 0) {
                console.log(`[Persistence] Restored ${restored} torrents`)
                this.emit('update', this.getTorrents())
            }
        } catch (err) {
            console.error('[Persistence] Restore error:', err.message)
        }
    }

    async _restoreSeed(torrentData) {
        const { key, sourcePath, name } = torrentData

        // Проверяем существует ли исходный путь
        if (!fs.existsSync(sourcePath)) {
            console.warn(`[Restore] Source path not found: ${sourcePath}, skipping seed`)
            return
        }
        ...
        // Создаём сессию с известным ключом
        const sessionStore = this.rootStore.namespace(key)
        const drive = new Hyperdrive(sessionStore, b4a.from(key, 'hex'))
        await drive.ready()
        ...
        const torrent = {
            ...
            controller,
            activeTransfer: false,
            fileCount: torrentData.fileCount || 0,
            sourcePath,
            appliedVersion: drive.version,
            createdAt: torrentData.createdAt,
            restored: true // Флаг что торрент восстановлен
        }
        ...
        // Добавляем в каталог поиска если это сид
        this.searchManager.addToCatalog(torrent)

        this.emit('update', this.getTorrents())

        console.log(`[Restore] Seed restored: ${torrent.name}`)
    }

    async _restoreDownload(torrentData) {
        const { key, saveDir, selectedFiles } = torrentData
        ...
        const torrent = {
            key,
            name: torrentData.name || 'Identifying...',
            size: torrentData.size || 0,
            type: torrentData.type || 'downloading',
            drive,
            store: sessionStore,
            discovery,
            peers: 0,
            progress: torrentData.progress || 0, // Восстанавливаем прогресс
            saveDir,
            controller,
            activeTransfer: false,
            appliedVersion: 0,
            files: torrentData.files || [],
            metadata: torrentData.metadata,
            fileCount: torrentData.fileCount || 0,
            selectedFiles,
            createdAt: torrentData.createdAt,
            restored: true
        ...
        this._monitorTorrent(key)
        this._monitorRemoteUpdates(key)

        // Если уже есть метаданные и файлы
        if (torrent.files && torrent.files.length > 0) {
            torrent.appliedVersion = drive.version

            // Если загрузка была в процессе и не завершена — возобновляем
            if (torrent.type === 'downloading' && torrent.progress < 100) {
                console.log(`[Restore] Resuming download for ${torrent.name}`)
                this._startDownload(key)
            }
        } else {
            // Загружаем метаданные если их нет
            this._fetchMetadata(key)
        }

        console.log(`[Restore] Download restored: ${torrent.name}`)
    }
    ...
    async restoreTorrents() {
        // Публичный метод для восстановления (вызывается после полной инициализации)
        await this._restoreTorrents()
    }

    // ==================== Rating Methods ====================

    async ratePeer(peerKey, rating) {
        return await this.ratingManager.ratePeer(peerKey, rating)
    }
    ...
    getTopPeers(limit = 10) {
        return this.ratingManager.getTopPeers(limit)
    }

    getRating(peerKey) {
        return this.ratingManager.getRating(peerKey)
    }

    async fetchPeerRatings(peerKey) {
        return await this.ratingManager.fetchPeerRatings(peerKey)
    }

    calculateTrustScore(peerKey) {
        return this.ratingManager.calculateTrustScore(peerKey)
    }

    isTrusted(peerKey, threshold = 70) {
        return this.ratingManager.isTrusted(peerKey, threshold)
    }

    async importRatings(ratings) {
        return await this.ratingManager.importRatings(ratings)
    }

    async exportRatings() {
        return await this.ratingManager.exportRatings()
    }

    getRatingsDriveKey() {
        return this.ratingManager.getRatingsDriveKey()
    }

    getMyRatingStats() {
        return this.ratingManager.getMyStats()
    }

    getMyPublicKey() {
        return this.ratingManager.getMyPublicKey()
    }

    getSeederKey(keyHex) {
        const torrent = this.torrents.get(keyHex)
        if (!torrent) return null
        return torrent.drive.key.toString('hex')
    }
