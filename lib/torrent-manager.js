...
export class TorrentManager extends EventEmitter {
    ...
    async _startDownload(keyHex) {
        ...
        try {
            ...
            // Ждём подключения хотя бы одного пира
            try {
            ...
            let metadata = null
            console.log(`[Download] Waiting for metadata file...`)

            // Ждём появления метаданных
            for (let i = 0; i < 15 && !metadata; i++) {
                ...
                try {
                    ...
                    if (metadataEntry) {
                        ...
                        console.log(`[Download] Metadata loaded: ${metadata.name}`)
                    }
                } catch (err) {
                    // Игнорируем ошибки, пробуем снова
                }
                if (!metadata) {
                    console.log(`[Download] Metadata not found, attempt ${i + 1}/15...`)
                    await new Promise(r => setTimeout(r, 1500))
                }
            }

            if (!metadata) {
                console.log(`[Download] No metadata found, using defaults`)
            }

            // Получаем список всех файлов (исключая метаданные)
            const entries = []
            ...
            // Используем название из метаданных или генерируем
            const torrentName = metadata?.name || `downloaded-${keyHex.slice(0, 6)}`
            ...
            if (isDirectory) {
                ...
                // Скачиваем в папку с названием раздачи
                const baseSavePath = path.join(torrent.saveDir, torrentName)
                ...
                // Для одного файла используем название из метаданных или ключ
                const fileName = metadata?.sourceName || `downloaded-${keyHex.slice(0, 6)}`
                const savePath = path.join(torrent.saveDir, fileName)
                let downloaded = 0
